...
export default function Home() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const { data, error } = await supabase.from("posts_new").select("id, image_url, caption, likes").gt("likes", 5).order("likes", { ascending: false });
      if (error) {
        console.error("Error fetching posts:", error);
      } else {
        console.log("Posts obtenidos:", data);
      }
    }; // Esto lo insertamos en la clase para llamar los posts desde Supabase
